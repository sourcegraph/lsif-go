...
func (i *indexer) index(pkgs []*packages.Package) (*Stats, error) {
	...
	for _, p := range pkgs {
		if err := i.indexPkg(p, proID); err != nil {
	...
	for _, f := range i.files {
		...
		if len(f.defRangeIDs) > 0 || len(f.useRangeIDs) > 0 {
			_, err = i.w.EmitContains(f.docID, append(f.defRangeIDs, f.useRangeIDs...))
			if err != nil {
...
func (i *indexer) indexPkg(p *packages.Package, proID string) (err error) {
	...
	for _, f := range p.Syntax {
		fpos := p.Fset.Position(f.Package)
		if err := i.indexUses(p, i.files[fpos.Filename], fpos.Filename); err != nil {
...
func (i *indexer) indexUses(p *packages.Package, fi *fileInfo, filename string) error {
	...
	for ident, obj := range p.TypesInfo.Uses {
		...
		if def.defResultID == "" {
			...
			_, err = i.w.EmitTextDocumentDefinition(def.resultSetID, defResultID)
			if err != nil {
			...
			_, err = i.w.EmitItem(def.defResultID, []string{def.rangeID}, def.docID)
			if err != nil {
				return fmt.Errorf(`emit "item": %v`, err)
			}
		}
